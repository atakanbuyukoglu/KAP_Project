...
print(company_name_upper, 'Hasılat (Son 12 Ay):', '{:,.2f}'.format(company.get_revenue(ttm=False)))
print(company_name_upper, 'Faaliyet Karı (Son 12 Ay, Diğer Hariç):', '{:,.2f}'.format(company.get_basic_operating_income(ttm=False)))
print(company_name_upper, 'Amortizasyon (Son 12 Ay):', '{:,.2f}'.format(company.get_amortization(ttm=False)))
print(company_name_upper, 'FAVÖK (Son 12 Ay):', '{:,.2f}'.format(company.get_ebitda(ttm=False)))
print(company_name_upper, 'Düzeltilmiş FAVÖK (Son 12 Ay):', '{:,.2f}'.format(company.get_adjusted_ebitda(ttm=False, control_adjusted=True)))
...
print(company_name_upper, 'Hisse Sayısı:', '{:,.2f}'.format(company.get_share_count(online=False)))
